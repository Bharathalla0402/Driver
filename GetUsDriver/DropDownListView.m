    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection)
        {
            if ([UIScreen mainScreen].bounds.size.width < 700 )
            {
                ...
                [btnDone setFrame:CGRectMake(rect.origin.x+190,10, 82, 31)];
            ...
            {
                UIButton *btnDone=[UIButton  buttonWithType:UIButtonTypeCustom];
                [btnDone setFrame:CGRectMake(rect.origin.x-40,10, 82, 31)];
                [btnDone setImage:[UIImage imageNamed:@"done@2x.png"] forState:UIControlStateNormal];
                [btnDone addTarget:self action:@selector(Click_Done) forControlEvents: UIControlEventTouchUpInside];
                [self addSubview:btnDone];
            }
...
-(void)Click_Done{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(DropDownListView:Datalist:)]) {
        ...
        for (int k=0; k<unique.count; k++) {
            ...
            NSIndexPath *path=[unique objectAtIndex:k];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *strlanguage=[[NSUserDefaults standardUserDefaults]objectForKey:@"strlanguage"];
    
    if ([strlanguage isEqualToString:@"Registration"])
    {
        ...
        cell.textLabel.frame=CGRectMake(55, 0, 120, 30);
    ...
    else
    {
        if (count==1)
        {
            ...
            NSData *data = [defaults objectForKey:@"languagedata"];
            ...
            {
                if(indexPath.row == [[arrnames objectAtIndex:i]integerValue])
                {
                    int row = (int)[indexPath row];
                    UIImageView *imgarrow=[[UIImageView alloc]init ];
                    imgarrow.frame=CGRectMake(10,5, 30, 30);
                    [imgarrow sd_setImageWithURL:[NSURL URLWithString:@""]
                                   placeholderImage:[UIImage imageNamed:@"remember Check2.png"]];
                    ...
                    {
                        // [imgarrow sd_setImageWithURL:[NSURL URLWithString:@""]
                        //       placeholderImage:[UIImage imageNamed:@"remember Check.png"]];
                    }
